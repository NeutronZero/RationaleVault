"""RationaleVault Knowledge Synthesizer — Core synthesis engine.
...
def synthesize_all(
    ...
    """Main entry point: scan project memories and produce knowledge objects.

    This is an explicit projection step, NOT automatic on append_event.
    Knowledge synthesis is triggered via:
        - rationalevault knowledge synthesize
        - run_handoff_suite()

    Args:
        project_id: The project UUID. Only memories belonging to this project
                    are synthesized. This enforces project boundary isolation.
        existing_knowledge: Previously synthesized knowledge (for STALE detection).
        synthesis_event_id: The event ID that triggered this synthesis run.
                           Preserves provenance chain: Event -> Knowledge.

    Returns:
        List of synthesized knowledge objects scoped to this project.
    """
    from rationalevault.memory.factory import get_memory_provider

    provider = get_memory_provider()
    all_memories = provider.get_all_records()
    pid_str = str(project_id)
    memories = [m for m in all_memories if m.project_id == pid_str]

    knowledge: list[KnowledgeObject] = []
    knowledge.extend(_detect_project_invariants(memories, pid_str, synthesis_event_id))
    knowledge.extend(_detect_architecture_principles(memories, pid_str, synthesis_event_id))
    knowledge.extend(_detect_lessons(memories, pid_str, synthesis_event_id))
    knowledge.extend(_detect_failure_patterns(memories, pid_str, synthesis_event_id))
    knowledge.extend(_detect_workflow_patterns(memories, pid_str, synthesis_event_id))
    knowledge.extend(_detect_research_findings(memories, pid_str, synthesis_event_id))
    knowledge.extend(_build_decision_lineage(memories, pid_str, synthesis_event_id))
...
def _detect_project_invariants(
    memories: list[MemoryRecord], project_id: str = "",
    synthesis_event_id: str = "",
) -> list[ProjectInvariant]:
...
def _detect_architecture_principles(
    memories: list[MemoryRecord], project_id: str = "",
    synthesis_event_id: str = "",
) -> list[ArchitecturePrinciple]:
...
def _compute_confidence(
    ...
    Args:
        memories: Supporting memories for this knowledge.
        contradicting_memory_ids: Optional list of memory IDs that contradict this knowledge.
                                  If provided, counted against confidence score.
    """
    if not memories:
    ...
    contradiction_count = len(contradicting_memory_ids) if contradicting_memory_ids else 0
...
def _mark_stale_knowledge(
    ...
    """Mark existing knowledge as STALE or SUPERSEDED based on current synthesis.

    If a current knowledge object shares supporting memories and same type,
    the existing one is marked SUPERSEDED with a pointer to the replacement.
    Otherwise, if no supporting memories overlap, marked STALE.
    """
    ...
    for existing_k in existing:
        # Find the best matching new knowledge by type + memory overlap
        best_match = None
        best_overlap = 0
        for new_k in current:
            if new_k.knowledge_type != existing_k.knowledge_type:
                continue
            overlap = set(existing_k.supporting_memory_ids) & set(new_k.supporting_memory_ids)
            if len(overlap) > best_overlap:
                best_overlap = len(overlap)
                best_match = new_k

        if best_match and best_overlap > 0:
            existing_k.lifecycle_status = KnowledgeLifecycle.SUPERSEDED.value
            existing_k.superseded_by = best_match.id
        elif not (set(existing_k.supporting_memory_ids) & current_memory_ids):
