"""RationaleVault ContextPackageCompiler — Abstract base for agent-specific context compilation.
...
Sibling abstraction to AgentCompiler (rationalevault/compilers/base.py):
  - AgentCompiler: CognitiveHead → agent state context
  - ContextPackageCompiler: ContextPackage → agent retrieval context

Design principles:
  - Output-only. No LLM API calls. RationaleVault is model-agnostic.
