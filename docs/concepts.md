...
Relay relies on a few simple, powerful concepts.
...
## 2. Event Sourcing
Traditional memory systems update databases directly (destructive updates). Relay records everything as an immutable stream of events (e.g., `PROJECT_GOAL_SET`, `DECISION_ACCEPTED`, `TASK_COMPLETED`). State is never modified; it is replayed.
