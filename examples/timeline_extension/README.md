# Projection Example (Timeline Extension)
...
## What it does
The `TimelineExtensionProjection` consumes `TaskCreatedEvent` and `TaskCompletedEvent` facts from the event stream and materializes a queryable `TimelineExtensionState`.
