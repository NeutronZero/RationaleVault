...
from relay.db.base import BaseEventStore
...
class SQLiteEventStore(BaseEventStore):
    ...
    def _init_db(self) -> None:
        ...
        with self._get_conn() as conn:
            ...
            """)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_relay_events_project ON relay_events(project_id);")
        ...
        def _run(c: sqlite3.Connection) -> EventRecord:
            ...
            try:
                ...
                cur = c.execute(
                    ...
                    "FROM relay_events WHERE project_id = ?",
                    (str(project_id),),
                )
    ...
    def get_event_count(self, project_id: UUID) -> int:
        sql = "SELECT COUNT(*) AS cnt FROM relay_events WHERE project_id = ?"
        with self._get_conn() as conn:
