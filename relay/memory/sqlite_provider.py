...
class SQLiteMemoryProvider(BaseMemoryProvider):
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS relay_memories (
                id TEXT PRIMARY KEY,
                version INTEGER,
                title TEXT,
                content TEXT,
        ...
        # Migration guard
        try:
            conn.execute("ALTER TABLE relay_memories ADD COLUMN reference_count INTEGER DEFAULT 0")
            conn.commit()
        except Exception:
            pass
        try:
            conn.execute("ALTER TABLE relay_memories ADD COLUMN last_referenced_at TEXT")
            conn.commit()
        except Exception:
            pass
        return conn
    ...
    def add_record(self, record: MemoryRecord) -> None:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT version, content FROM relay_memories WHERE id = ?", (record.id,))
            row = cursor.fetchone()
            if row:
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO relay_memories (
