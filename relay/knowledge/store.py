"""Relay Knowledge Store — Persistence layer for synthesized knowledge.
...
from relay.knowledge.models import KnowledgeObject, KnowledgeType, KnowledgeDomain
...
class SQLiteKnowledgeProvider(BaseKnowledgeProvider):
    ...
    def __init__(self, db_path: str | Path = None) -> None:
        if db_path is None:
            self.db_path = Path.cwd() / ".relay" / "relay.db"
        else:
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS relay_knowledge (
                id TEXT PRIMARY KEY,
                version INTEGER,
                title TEXT,
                content TEXT,
    ...
    def add_knowledge(self, knowledge: KnowledgeObject) -> None:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT version, content FROM relay_knowledge WHERE id = ?",
                (knowledge.id,),
            )
            row = cursor.fetchone()
            if row:
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO relay_knowledge (
    ...
    def update_lifecycle(self, knowledge_id: str, status: str) -> None:
        ...
        try:
            conn.execute(
                "UPDATE relay_knowledge SET lifecycle_status = ?, updated_at = datetime('now') WHERE id = ?",
                (status, knowledge_id),
            )
            conn.commit()
        finally:
    ...
    def _row_to_knowledge(self, row: tuple) -> KnowledgeObject:
        from relay.knowledge.models import KnowledgeConfidence, ProvenanceChain
        d = {
            "id": row[0],
            "version": row[1],
            "title": row[2],
...
class MarkdownKnowledgeProvider(BaseKnowledgeProvider):
    ...
    def __init__(self, file_path: str | Path = None) -> None:
        if file_path is None:
            self.file_path = Path.cwd() / ".relay" / "knowledge.md"
        else:
    ...
    def _ensure_file(self) -> None:
        ...
        if not self.file_path.exists():
            with open(self.file_path, "w", encoding="utf-8") as f:
                f.write("# Relay Knowledge Corpus\n\n")
