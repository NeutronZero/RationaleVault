"""Relay Unified Evaluator — Pipeline to evaluate the quality and health of the entire cognitive platform."""
...
from relay import __version__ as RELAY_VERSION, SCHEMA_VERSION
...
def run_full_evaluation() -> EvaluationResult:
    ...
    from relay.knowledge.models import KnowledgeObject, KnowledgeType, KnowledgeDomain, KnowledgeConfidence, ProvenanceChain
    ...
    try:
        ...
        if hasattr(mem_prov, "_get_conn"):
            ...
            try:
                conn.execute("DELETE FROM relay_memories WHERE id LIKE 'eval_temp_%'")
                conn.commit()
            except Exception:
                pass
            finally:
        ...
        if hasattr(k_prov, "_get_conn"):
            ...
            try:
                conn.execute("DELETE FROM relay_knowledge WHERE id LIKE 'eval_temp_%'")
                conn.commit()
            except Exception:
                pass
            finally:
        ...
        if hasattr(event_store, "_store") and hasattr(event_store._store, "_get_conn"):
            ...
            try:
                conn.execute("DELETE FROM relay_events WHERE project_id = ?", (str(project_uuid),))
                conn.commit()
            except Exception:
                pass
            finally:
    ...
    # 6. Examples Verification (Run Examples)
    import sys
    from pathlib import Path
    ...
    examples_status = {}
    try:
        from examples.basic_memory.main import main as run_basic_memory
    ...
    try:
        from examples.knowledge_synthesis.main import main as run_knowledge_synthesis
    ...
    try:
        from examples.multi_agent_handoff.main import main as run_multi_agent_handoff
    ...
    reports_dir = Path.cwd() / ".relay" / "reports"
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write(f"# Relay Platform Evaluation Summary (v{RELAY_VERSION})\n\n")
